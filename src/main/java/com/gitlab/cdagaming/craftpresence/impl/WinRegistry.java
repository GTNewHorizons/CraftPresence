...
public class WinRegistry {
    ...
    static {
        try {
            ...
            regOpenKey = userClass.getDeclaredMethod(
                    "WindowsRegOpenKey", (javaSpec >= 11 ? long.class : int.class), byte[].class, int.class);
            regOpenKey.setAccessible(true);
            regCloseKey = userClass.getDeclaredMethod("WindowsRegCloseKey", (javaSpec >= 11 ? long.class : int.class));
            regCloseKey.setAccessible(true);
            regQueryValueEx = userClass.getDeclaredMethod(
                    "WindowsRegQueryValueEx", (javaSpec >= 11 ? long.class : int.class), byte[].class);
            regQueryValueEx.setAccessible(true);
            regEnumValue = userClass.getDeclaredMethod(
                    "WindowsRegEnumValue", (javaSpec >= 11 ? long.class : int.class), int.class, int.class);
            regEnumValue.setAccessible(true);
            regQueryInfoKey =
                    userClass.getDeclaredMethod("WindowsRegQueryInfoKey1", (javaSpec >= 11 ? long.class : int.class));
            regQueryInfoKey.setAccessible(true);
            regEnumKeyEx = userClass.getDeclaredMethod(
                    "WindowsRegEnumKeyEx", (javaSpec >= 11 ? long.class : int.class), int.class, int.class);
            regEnumKeyEx.setAccessible(true);
            regCreateKeyEx = userClass.getDeclaredMethod(
                    "WindowsRegCreateKeyEx", (javaSpec >= 11 ? long.class : int.class), byte[].class);
            regCreateKeyEx.setAccessible(true);
            regSetValueEx = userClass.getDeclaredMethod(
                    "WindowsRegSetValueEx", (javaSpec >= 11 ? long.class : int.class), byte[].class, byte[].class);
            regSetValueEx.setAccessible(true);
            regDeleteValue = userClass.getDeclaredMethod(
                    "WindowsRegDeleteValue", (javaSpec >= 11 ? long.class : int.class), byte[].class);
            regDeleteValue.setAccessible(true);
            regDeleteKey = userClass.getDeclaredMethod(
                    "WindowsRegDeleteKey", (javaSpec >= 11 ? long.class : int.class), byte[].class);
    ...
    private static int deleteKey(Preferences root, int hkey, String key)
            throws IllegalArgumentException, IllegalAccessException, InvocationTargetException {
        return (int) (Integer) regDeleteKey.invoke(
                root, new Object[] {hkey, toCstr(key)}); // can REG_NOTFOUND, REG_ACCESSDENIED, REG_SUCCESS
    }
    ...
    private static List<String> readStringSubKeys(Preferences root, int hkey, String key)
        ...
        int count = info[
                0]; // Fix: info[2] was being used here with wrong results. Suggested by davenpcj, confirmed by Petrucio
        int maxlen = info[3]; // value length max
        for (int index = 0; index < count; index++) {
